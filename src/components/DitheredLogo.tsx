...
const toGrayscaleGrid = (
  ...
  if (!alphaCtx) throw new Error("DitheredLogo: alpha ctx");
  ...
  if (!srcCtx) throw new Error("DitheredLogo: src ctx");
  ...
  if (!ctx) throw new Error("DitheredLogo: proc ctx");
...
const initParticles = (
  ...
  return { count, baseX, baseY, offsetX, offsetY, brightness, tint, size: scaleFactor * dotScale };
};
...
export function DitheredLogo({
  ...
  const resolveParticleColor = useCallback(() => {
    ...
    return particleColor === "currentColor" ? getComputedStyle(canvas).color : particleColor;
  }, [particleColor]);
  ...
  const startLoop = useCallback(() => {
    ...
    const tick = () => {
      ...
      const needsMore = stepParticles(
        sys, cursorRef.current.x, cursorRef.current.y, cursorRef.current.active,
        ripplesRef.current, performance.now(),
      );

      drawParticles(ctx, sys, resolveParticleColor(), rect.width, rect.height, dpr);

      if (needsMore) animFrameRef.current = requestAnimationFrame(tick);
    ...
    async (src: string) => {
      ...
      try {
        ...
        let positions = errorDiffusionDither(
          processed.grayscale, gridW, gridH,
          { threshold, serpentine, diffusionStrength }, processed.alpha,
        );

        if (invert) {
          positions = applyMaskInversion(positions, gridW, gridH, cornerRadius, processed.alpha);
        }
        ...
        systemRef.current = initParticles(positions, scaleFactor, responsiveDotScale, originX, originY);
        startLoop();
      } catch (error) {
        console.error("DitheredLogo: failed to process image", error);
      }
    },
    [gridSize, scale, dotScale, invert, cornerRadius, threshold, contrast, gamma, blur, diffusionStrength, serpentine, isMobile, startLoop],
  );

  useEffect(() => {
    const key = JSON.stringify([
      imageSrc, gridSize, scale, dotScale, invert, cornerRadius,
      threshold, contrast, gamma, blur, diffusionStrength, serpentine, isMobile,
    ]);
    if (key === prevConfigRef.current) return;
    prevConfigRef.current = key;
    rebuild(imageSrc);
  }, [imageSrc, gridSize, scale, dotScale, invert, cornerRadius, threshold, contrast, gamma, blur, diffusionStrength, serpentine, isMobile, rebuild]);
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      const sys = systemRef.current;
      if (sys) drawParticles(ctx, sys, resolveParticleColor(), rect.width, rect.height, dpr);
    ...
    const onPointerCancel = () => { cursorRef.current.active = false; startLoop(); };
